 ...
 */
class QuickBooks_Driver_Sql_Pdo extends QuickBooks_Driver_Sql
{
	...
	public function __construct($dsn_or_conn, $config)
	{
		...
		if (is_resource($dsn_or_conn) or ($dsn_or_conn instanceof PDO))
		{
    ...
    try {
      if ($port) {
        $this->_conn = new PDO("mysql:host=$host;dbname=$db;port=$port", $user, $pass);
      } else {
        $this->_conn = new PDO("mysql:host=$host;dbname=$db", $user, $pass);
      }
    } catch(PDOException $e) {
      trigger_error("host: $host, user: $user, pass: $pass, message: {$e->getMessage()}", E_USER_ERROR);
    }
	...
	protected function _query($sql, &$errnum, &$errmsg, $offset = 0, $limit = null)
	{
		...
		if (!$res)
		{
			$errnum = $this->_conn->errorCode();
			$errmsg = $this->_conn->errorInfo();
			$this->_last_error = $this->_conn->errorCode();
	...
	{
    trigger_error('PDO driver does not support \'rewind\' action.', E_USER_ERROR);
		return true;
	}	
